	...
	while (1) {
		if (timer_data_ready_count > 0) {
			...
			for (int i = 0; i < available_samples; i++) {
				...
				// 25 Hz: PPG BLE transmission (decimated by 32 from 800Hz)
...
static void MX_GPIO_Init(void) {
	...
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_3 | BLE_P0_0_Pin | BLE_P3_6_Pin | BLE_UART_RX_IND_Pin | BLE_RESET_Pin, GPIO_PIN_RESET);
	...
	/*Configure GPIO pins : PC3 BLE_P0_0_Pin BLE_P3_6_Pin BLE_UART_RX_IND_Pin
							 BLE_RESET_Pin */
	GPIO_InitStruct.Pin = GPIO_PIN_3 | BLE_P0_0_Pin | BLE_P3_6_Pin | BLE_UART_RX_IND_Pin | BLE_RESET_Pin;
...
void HAL_GPIO_EXTI_Rising_Callback(uint16_t GPIO_Pin) {
	if (GPIO_Pin == USER_BUTTON_Pin) {
		...
		case STATE_ACQUISITION:
			...
			if (sample > 0) {
				for (uint16_t i = sample; i < 4096; i++)
					NAND_packet[i] = 0xFF;
				sample = 4096;
				write_memory();
			}
